# Sprint 2 - Global Design System

## Goal
Implement strict Swiss/International Typographic Style foundation.

## Tasks
- Implement 12-column CSS grid
- Define typography scale
- Set color tokens: black #000, white #FFF, neutral gray, one accent color
- Define spacing system (8px base unit)

## Files
- assets/css/styles.css

## Acceptance Criteria
- Layout uses consistent grid columns
- All content left-aligned
- Only 1-2 typefaces used
- No decorative shadows, gradients, or rounded UI
- Accent color used consistently

## Verification Steps
- Overlay grid in dev tools to confirm alignment
- Inspect computed styles to match typography scale
- Search CSS to ensure only defined color tokens are used

## What NOT to Do
- No centered hero sections
- No decorative dividers
- No animations
